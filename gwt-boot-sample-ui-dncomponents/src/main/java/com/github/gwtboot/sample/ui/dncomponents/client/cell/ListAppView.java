...
public class ListAppView implements IsElement, HasCellComponents {
	...
	private void init() {
		list.getSelectionModel()
				.addSelectionHandler(evt -> evt.getSelectedItem().forEach(p -> logTa.append(p.getId() + "\n")));
		...
		listPeople.getSelectionModel().addSelectionHandler(
				evt -> evt.getSelectedItem().forEach(p -> logTa2.append(p.getName() + " " + p.getAge() + "\n")));
